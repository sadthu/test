$('.insert_attach').click(function() {
    if ($('.list_attach').hasClass('show-btn') === false) {
        $('.list_attach').addClass('show-btn');
    }
    var _lastimg = jQuery('.attach_view li').last().find('input[type="file"]').val();
    ...
    if (_lastimg != '') {
        ...
        _html += '</div>';
        _html += '</li>';
        jQuery('.attach_view').append(_html);
        jQuery('.attach_view li').last().find('input[type="file"]').click();
    } else {
        if (_lastimg == '') {
            jQuery('.attach_view li').last().find('input[type="file"]').click();
        } else {
